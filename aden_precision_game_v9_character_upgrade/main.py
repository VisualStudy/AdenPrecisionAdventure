...
class HumanBoss(BossEnemy):
    """Fast melee human boss. Map symbol: N"""

    def __init__(self, x, y):
        super().__init__(x - 16, y - 42, HUMAN_BOSS_FRAMES, 32)
        self.base_speed = 3.8
        self.attack_timer = 0
        self.dash_timer = 0
        self.target_x = x
    ...
    def update(self, solids, bullets, player):
        ...
        if abs(dx) < 150 and abs(dy) < 90 and self.attack_timer <= 0:
            self.dash_timer = 18
        ...
        # Keep inside rough level space by resolving solids.
        for tile in pygame.sprite.spritecollide(self, solids, False):
            if dx > 0:
                self.rect.right = tile.rect.left
            else:
